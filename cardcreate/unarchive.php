...
try {
    ...
    $update = $DB->query("UPDATE members SET archived = 0 WHERE idMembers = :id", [':id' => $memberId]);

    if ($update) {
        echo json_encode(['status' => 'success', 'message' => 'Membre désarchivé avec succès.']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Impossible de désarchiver le membre.']);
    }
